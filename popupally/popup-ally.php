	...
	class PopupAlly {
		...
		public static function enqueue_resources() {
			...
			if (!empty($to_show)) {
				...
				wp_enqueue_script('popupally-action-script', self::$PLUGIN_URI . 'resource/frontend/popup.min.js', array('jquery'), self::VERSION, false);
			...
			if (!empty($ids)){
				foreach($ids as $id) {
					wp_enqueue_script('popupally-thank-you-script-' . $id, $base_script_url . '/' . $script_prefix . 'popupally-thank-you-' . $id . '.js', false, self::VERSION . '.' . $num_saved, false);
				}
			}
		}
		...
		public static function enqueue_administrative_resources($hook) {
			if (strpos($hook, self::SETTING_KEY_ALL) !== false) {
				...
				wp_enqueue_script('popupally-backend-default-code', self::$PLUGIN_URI . 'resource/backend/js/popup-default-code.js', false, self::VERSION, false);
				wp_enqueue_script('popupally-backend', self::$PLUGIN_URI . 'resource/backend/js/popupally.min.js', array('jquery', 'popupally-backend-default-code'), self::VERSION, false);
				wp_enqueue_script('popupally-backend-color-picker', self::$PLUGIN_URI . 'resource/backend/jscolor/jscolor.js', array('jquery'), self::VERSION, false);
		...
		public static function add_preview_popup_scripts() {
			...
			foreach($style_settings as $id => $style) {
				echo '<style id="popupally-preview-css-' . esc_attr($id) . '" type="text/css">';
				$template_uid = $style['selected-template'];
				$template_obj = self::get_template($template_uid);
				if ($template_obj) {
					$css_code = PopupAllyStyleCodeGeneration::generate_popup_css($id, $style, $style_settings, 1, $template_obj);
					echo do_shortcode($css_code);
				}
				echo '</style>';
			}
		}
		...
		private static function sanitize_popup_code($code) {
			if (defined('DISALLOW_UNFILTERED_HTML') && DISALLOW_UNFILTERED_HTML) {
				// remove all <script> code
				...
				foreach($to_show as $id => $popup_types) {
					if (in_array('embedded', $popup_types)) {
						if ('page-end' === $display[$id]['embedded-location']) {
							echo do_shortcode(self::sanitize_popup_code($code[$id]['embedded_html']));
						}
					}
					echo do_shortcode(self::sanitize_popup_code($code[$id]['html']));
				}
			}
		}
